(() => {
  ...
  const els = {
    ...
    helpSettingBtn: $("#helpSettingBtn"),
    helpModal: $("#helpModal"),
    helpCreateAllBtn: $("#helpCreateAllBtn"),
  ...
  function helpExampleDefinitions() {
    const strategyNodes = [
      { id: "cafe-skill", perspective: "learning", text: "提升員工數位工具能力", kpi: "一個月內完成 100% 數位點餐訓練" },
      { id: "cafe-data", perspective: "learning", text: "建立顧客與銷售資料能力", kpi: "每週完成一次銷售資料檢視" },
      { id: "cafe-order", perspective: "process", text: "縮短點餐與出餐流程", kpi: "平均出餐時間降至 8 分鐘" },
      { id: "cafe-menu", perspective: "process", text: "改善菜單與推薦內容", kpi: "每月依銷售資料調整一次菜單" },
      { id: "cafe-wait", perspective: "customer", text: "降低顧客等待感", kpi: "等待體驗滿意度達 4.5／5" },
      { id: "cafe-value", perspective: "customer", text: "提高個人化與便利性", kpi: "會員推薦點擊率達 25%" },
      { id: "cafe-repeat", perspective: "financial", text: "提升顧客回購率", kpi: "六個月內回購率提升 15%" },
      { id: "cafe-revenue", perspective: "financial", text: "增加穩定營收", kpi: "六個月內月營收成長 10%" }
    ];
    const strategyEdges = [
      ["cafe-skill", "cafe-order", "支援"],
      ["cafe-data", "cafe-menu", "支援"],
      ["cafe-order", "cafe-wait", "降低"],
      ["cafe-menu", "cafe-value", "提升"],
      ["cafe-wait", "cafe-repeat", "促進"],
      ["cafe-value", "cafe-repeat", "促進"],
      ["cafe-repeat", "cafe-revenue", "帶動"]
    ].map(([source, target, label], index) => ({ id: `cafe-edge-${index + 1}`, source, target, label }));

    return {
      knowledge: {
        title: "範例｜睡眠與學習的 Knowledge Graph",
        content: [
          "# 睡眠如何影響學習", "", "這篇筆記示範如何用概念關係整理一個研究主題。", "",
          "## 主要概念", "", "- 睡眠品質", "- 注意力", "- 記憶鞏固", "- 學習成效", "",
          "```knowledge",
          "規律作息 | 改善 | 睡眠品質",
          "睡眠品質 | 提升 | 注意力",
          "睡眠品質 | 促進 | 記憶鞏固",
          ...
          "睡眠不足 | 增加 | 疲勞",
          "疲勞 | 降低 | 注意力",
          "```", "", "按上方 ◎ 查看目前筆記的 Knowledge Graph。", "", "#教學 #KnowledgeGraph"
        ].join("\n")
      },
      mind: {
        title: "範例｜地方博物館展覽心智圖",
        content: [
          "# 地方博物館展覽企劃", "", "## 核心主題", "", "- 城市的日常記憶", "  - 市場", "  - 學校", "  - 交通", "  - 家庭生活", "",
          "## 展示內容", "", "- 歷史照片", "  - 街景", "  - 人物", "- 實體文物", "  - 車票", "  - 校服", "  - 家庭用品", "- 口述歷史", "  - 老居民", "  - 商家", "  - 教師", "",
          "## 觀眾體驗", "", "- 語音導覽", "- 互動地圖", "- 回憶留言牆", "- 親子學習單", "",
          "## 執行工作", "", "- 文物徵集", "- 版權確認", "- 空間設計", "- 宣傳與教育活動", "", "按上方 ⌘ 查看心智圖。", "", "#教學 #心智圖"
        ].join("\n")
      },
      flow: {
        title: "範例｜圖書館借書申請流程圖",
        content: [
          "# 圖書館借書流程", "", "以下示範一般步驟、判斷、是／否分支與循環。", "",
          "```flow",
          "開始((開始)) -> 搜尋[搜尋館藏]",
          "搜尋 -> 有館藏{館藏可借？}",
          "有館藏 ->|否| 預約[加入預約名單]",
      ...
      tree: {
        ...
        content: [
          ...
          "      蕨類",
          "      苔蘚",
          "```", "", "按上方 ♧ 查看樹狀圖，並點擊有子節點的方塊測試收合。", "", "#教學 #樹狀圖"
        ].join("\n")
      },
      strategy: {
        title: "範例｜咖啡店數位轉型策略地圖",
        content: [
          "# 咖啡店數位轉型策略", "", "## 策略主題", "", "透過員工能力與資料能力，改善點餐及菜單流程，進而提升顧客體驗、回購率與營收。", "",
          "## 建議閱讀方式", "", "從策略地圖最下方往上看：", "", "1. 學習與成長：培養數位工具與資料能力。", "2. 內部流程：改善點餐、出餐及菜單推薦。", "3. 顧客價值：降低等待感並提高便利性。", "4. 成果：增加回購率與穩定營收。", "", "按上方 ⇧ 開啟策略地圖。這篇範例已經包含可編輯的目標、KPI 與因果箭頭。", "", "#教學 #策略地圖"
        ].join("\n"),
        strategyMap: { perspectives: defaultPerspectives(), nodes: strategyNodes, edges: strategyEdges }
      }
    };
  }
...
開啟上方的「⇧」後，可在四個構面新增目標並建立因果箭頭。`,
  ...
  async function createNote(title = "未命名筆記", content = "") {
    ...
    requestAnimationFrame(() => {
      els.noteList.scrollTo({ top: 0, behavior: "smooth" });
  ...
  async function createHelpExample(kind, quiet = false) {
    const definition = helpExampleDefinitions()[kind];
    if (!definition) return null;
    await saveEditorNow();
    const existing = findByTitle(definition.title);
    if (existing) {
      if (!quiet) {
        closeModals();
        await openNote(existing.id);
        showToast("範例筆記已存在，已為你開啟");
      }
      return existing;
    }
    ...
    const note = normalizeNote({
      ...
      createdAt: now,
      updatedAt: now,
      strategyMap: definition.strategyMap || defaultStrategyMap()
    });
    state.notes.unshift(note);
    currentId = note.id;
    localStorage.setItem(CURRENT_KEY, currentId);
  ...
  async function createAllHelpExamples() {
    await saveEditorNow();
    const definitions = helpExampleDefinitions();
    let added = 0;
    let firstNew = null;
    const now = new Date().toISOString();

    for (const [kind, definition] of Object.entries(definitions)) {
      if (findByTitle(definition.title)) continue;
      const note = normalizeNote({
        id: uid(),
        title: definition.title,
        content: definition.content,
    ...
    const fallback = Object.values(definitions).map(item => findByTitle(item.title)).find(Boolean);
    const target = firstNew || fallback;
    if (target) {
      currentId = target.id;
      localStorage.setItem(CURRENT_KEY, currentId);
    }
    ...
    renderAll();
    closeModals();
    showToast(added ? `已建立 ${added} 篇教學範例` : "完整範例包已經存在");
  }
  ...
  async function openNote(id) {
    ...
    renderAll();
    requestAnimationFrame(() => scrollActiveNoteIntoView("smooth"));
    closeSidebar();
  ...
  function updateSidebarScrollState() {
    if (!els.noteList || !els.sidebarTopBtn) return;
    const maxScroll = Math.max(0, els.noteList.scrollHeight - els.noteList.clientHeight);
    const canScroll = maxScroll > 4;
    const isScrolled = els.noteList.scrollTop > 6;
    els.sidebar.classList.toggle("can-scroll", canScroll);
    els.sidebar.classList.toggle("scrolled", isScrolled);
    els.sidebarTopBtn.classList.toggle("hidden", !canScroll || els.noteList.scrollTop < 140);
  }

  function scrollActiveNoteIntoView(behavior = "auto") {
    const active = els.noteList?.querySelector(".note-item.active");
    if (!active || !els.noteList) {
      updateSidebarScrollState();
      return;
    }

    const listRect = els.noteList.getBoundingClientRect();
    const itemRect = active.getBoundingClientRect();
    const padding = 10;

    if (itemRect.top < listRect.top + padding) {
      const target = els.noteList.scrollTop + itemRect.top - listRect.top - padding;
      els.noteList.scrollTo({ top: Math.max(0, target), behavior });
    } else if (itemRect.bottom > listRect.bottom - padding) {
      const target = els.noteList.scrollTop + itemRect.bottom - listRect.bottom + padding;
      els.noteList.scrollTo({ top: target, behavior });
    }
    requestAnimationFrame(updateSidebarScrollState);
  }
  ...
  function renderNoteList() {
    ...
    if (els.noteCount) {
      els.noteCount.textContent = query
        ? `${filtered.length}／${state.notes.length} 篇筆記`
        : `${state.notes.length} 篇筆記`;
    }
  ...
  function bindEvents() {
    ...
    els.emptyHelpBtn.addEventListener("click", openHelpModal);
    els.searchInput.addEventListener("input", renderNoteList);
    els.noteList.addEventListener("scroll", updateSidebarScrollState, { passive: true });
    els.sidebarTopBtn.addEventListener("click", () => {
      els.noteList.scrollTo({ top: 0, behavior: "smooth" });
      els.noteList.focus({ preventScroll: true });
    });
    ...
    els.helpSettingBtn.addEventListener("click", openHelpModal);
    els.helpCreateAllBtn.addEventListener("click", createAllHelpExamples);
    $$(".help-example-btn").forEach(button => button.addEventListener("click", () => createHelpExample(button.dataset.helpExample)));
  ...
  async function init() {
    ...
    bindEvents();
    renderAll();
    requestAnimationFrame(() => scrollActiveNoteIntoView("auto"));
    await renderStorageStatus();
